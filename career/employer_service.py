...
class EmployerService(osv.AbstractModel):
	...
	def getCompany(self):
		...
		if users:
			return {'id': users[0].company_id.id, 'name': users[0].company_id.name,
				'image': users[0].company_id.logo or False}
		return False
	...
	def getAssignmentCandidate(self, assignmentId):
		for interview in self.env['survey.survey'].search(
			[('assignment_id', '=', assignmentId), ('state', '=', 'published')]):
			applicants = self.env['hr.applicant'].search(
				[('job_id', '=', assignmentId), ('active_survey_id', '=', interview.id)])
	...
	def createInterview(self, assignmentId, vals):
		if self.env['survey.survey'].search([('assignment_id', '=', assignmentId), ('state', '=', 'published')]):
			return False
	...
	def getInterview(self, assignmentId):
		interviewList =[]
		for interview in self.env['survey.survey'].search([('job_id','=',assignmentId)]):
			interviewList.append( {'id': interview.id, 'name': interview.title,
					 ...
					 'prepare': interview.prepare,'job_id':assignmentId,'round':interview.round,
	...
	def submitAssessment(self, assessmentResult):
		...
		hr_interview_assessment = self.env['hr.evaluation.interview'].search(
			[('user_id', '=', uid), ('applicant_id', '=', int(assessmentResult['candidateId']))])
	...
	def getSelfAssessment(self, assessmentId, applicantId):
		cr, uid, context = self.env.args
		hr_interview_assessment = self.env['hr.evaluation.interview'].search(
			[('applicant_id', '=', applicantId), ('user_id', '=', uid)])
	...
	def getAassignmentStatistic(self, assignmentId):
		...
		response_count = self.env['survey.user_input'].search_count(
			[('survey_id', '=', assignment.survey_id.id), ('state', '=', 'done')])
