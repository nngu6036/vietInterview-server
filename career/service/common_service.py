...
class CommonService(osv.AbstractModel):
	...
	@api.model
	def searchJob(self, keyword, options, start=None, length=None, count=False, company=None):
		assignmentList = []
		...
		if count:
			for a in self.env['hr.job'].search(domain):
				if categoryId and a.category_ids and not categoryId in a.category_ids.ids:
					continue
				if company and a.company_id.partner_id.email != company:
					continue
				totalTal = totalTal + 1

		for a in self.env['hr.job'].search(domain, limit=length, offset=start, order='create_date desc'):
			if categoryId and a.category_ids and not categoryId in a.category_ids.ids:
				continue
			if company and a.company_id.partner_id.email != company:
				continue
