...
class EmployeeUser(models.Model):
	...
	def getProfile(self):
		...
		return {'id':partner.id, 'name':partner.name,'phone':partner.phone,'mobile':partner.mobile,
				...
				'provinceId':partner.state_id.id,'birthdate':partner.birthdate or False,'image':partner.image or False,
			...
			if assignment.isEnabled():
				for survey in assignment.survey_ids:
					if survey.status =='published':
						user_input = self.env['survey.user_input'].search([('email','=',self.user_id.login),('survey_id','=',survey.id)])
						if not user_input:
						...
						candidate = self.env['hr.applicant'].search([('email_from','=',self.user_id.login),('job_id','=',assignment.id),
																	 ('join_survey_id','=',survey.id)])
						if not candidate:
							self.env['hr.applicant'].create({'name':self.name,'email_from':self.user_id.login,'job_id':assignment.id,
	...
	def addDocument(self,title,filename,file_location):
		doc = self.env['ir.attachment'].create({'name':title,'description':title,'res_model':'career.employee','res_id':self.id,
