...
wb = open_workbook('E:\\tmp\question4.xlsx')
...
for s in wb.sheets():
        for row in range(s.nrows):
            ...
            if category:
                ...
                prev_cat = 'career.question_category_%d_vi' % i
                print "<record model='career.question_category' id='%s'>" %prev_cat
                print "<field name='title'>%s</field>"%category
                print "<field name='lang'>vi</field>"
                print "</record>"
                i = i+1
            question = (s.cell(row,1).value).strip()
            print "<record model='career.question' id='career.question_%d_vi'>" %j
            j= j+1
